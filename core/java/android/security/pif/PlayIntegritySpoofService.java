...
public final class PlayIntegritySpoofService {
    ...
    private static final String DEFAULT_PIF_CONFIG_JSON = "{\n"
            + "  \"BRAND\": \"google\",\n"
            + "  \"DEVICE\": \"komodo\",\n"
            + "  \"MODEL\": \"Pixel 9 Pro XL\",\n"
            + "  \"MANUFACTURER\": \"Google\",\n"
            + "  \"FINGERPRINT\": \"google/komodo_beta/komodo:CANARY/ZP11.260417.009/15372612:user/release-keys\",\n"
    ...
    private void loadConfigInternal() {
        ...
        final boolean isPixelDevice = SystemProperties.get("ro.soc.manufacturer")
                .equalsIgnoreCase("Google");
        final boolean isPifEnabled = pifEnabledValue == null
                ? !isPixelDevice
                : ("1".equals(pifEnabledValue)
                || "true".equalsIgnoreCase(pifEnabledValue));
        if (!isPifEnabled) {
            mBuildFields.clear();
            mSystemProps.clear();
            mConfigLoaded = false;
        ...
        if (TextUtils.isEmpty(content) || content.trim().isEmpty()) {
            effectiveContent = DEFAULT_PIF_CONFIG_JSON;
            if (mVerboseLogs > 0) {
                Log.w(TAG, "No PIF config in Settings.Secure, using built-in default profile");
            }
        } else {
        ...
        try {
            ...
            String trimmed = effectiveContent.trim();
            if (trimmed.startsWith("{")) {
                parseJson(effectiveContent);
            } else {
                parseProp(effectiveContent);
            }
