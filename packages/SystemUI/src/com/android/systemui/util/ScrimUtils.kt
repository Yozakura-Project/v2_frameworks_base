...
class ScrimUtils private constructor(context: Context?) {
    ...
    fun setQsVisible(visible: Boolean) {
        if (mQsVisible.getAndSet(visible) != visible) {
            ...
            // Restore depth wallpaper when QS is closed on lockscreen
    ...
    fun setPulsing(pulsing: Boolean) {
        if (mPulsing.getAndSet(pulsing) != pulsing) {
            ...
            // Update depth wallpaper when exiting pulse/AOD
    ...
    fun onStartedWakingUp() {
        ...
        // Update depth wallpaper when waking up from AOD/ambient
        if (mStateIsKeyguard) {
            mainHandler.postDelayed({
            }, 150)
        }
    ...
    fun setQsExpansion(expansion: Float) {
        ...
        if (fullyCollapsed) {
            // Restore depth wallpaper when QS fully collapses on lockscreen
            if (mStateIsKeyguard) {
            }
        } else {
        }
