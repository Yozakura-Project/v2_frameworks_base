...
/**
 * Bridges the spoof configuration held in Settings.Secure (PlayIntegrityFix,
 * GameProps and TrickyStore) to the in-process consumers that read it back over
 * IActivityManager: android.security.pif.PlayIntegritySpoofService,
 * android.security.gameprops.GamePropsSpoofService and the keystore2 hook in
 * android.security.trickystore.TrickyStoreService.
 *
 * Ported from AxionOS. The service-injector plumbing (AxExtServiceFactory /
 * NtServiceInjector) is not used on YozakuraOS; ActivityManagerService owns the
 * single instance directly.
 *
 * @hide
 */
public class AxSpoofManager {
    ...
    private static final String[] WATCHED_KEYS = {
            ...
            Settings.Secure.SPOOF_PIF_ENABLED,
    ...
    public AxSpoofManager(Context context) {
        mContext = context;
    ...
    public String getPifEnabled() {
        return getCached(Settings.Secure.SPOOF_PIF_ENABLED);
    }
