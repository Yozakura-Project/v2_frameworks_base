...
final class ActivityRecord extends WindowToken {
    ...
    private int getStartingWindowType(boolean newTask, boolean taskSwitch, boolean processRunning,
        ...
        final boolean isAppLockerActivity =
        ...
        if (AxSandboxService.get().isAppLocked(this) || isAppLockerActivity) {
            return (isAppLockerActivity || processRunning)
                    ? STARTING_WINDOW_TYPE_NONE : STARTING_WINDOW_TYPE_SPLASH_SCREEN;
        }
    ...
    private boolean allowTaskSnapshot() {
        ...
        // Never snapshot a locked app or the unlock prompt: the snapshot would show the
        // contents in recents and in the starting window without any authentication.
        if (AxSandboxService.get().isAppLocked(this)
                || AxSandboxService.get().isAppLockerActivity(this.intent.getComponent())) {
            return false;
        }
